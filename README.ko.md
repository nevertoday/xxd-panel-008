...
| 모드 | 크기 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 원본 적응형 | 위에는 전체 원본, 아래에는 008 파스텔 불가능 공간. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `left-right` | 원본 적응형 | 왼쪽에는 전체 원본, 오른쪽에는 008 파스텔 불가능 공간. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `design-only` | 원본 적응형 | 원본 사진은 보이지 않고 변환 디자인만 표시. 원본 비율과 크기 유지 |
| `wallpaper-pack` | 기기별 네 가지 크기 | 휴대전화, iPad, 데스크톱, 어린이용 스마트워치 PNG 개별 출력 |

정확한 사용자 픽셀 > 지정 비율 또는 용도 > 일반 모드의 원본 적응형 순서입니다. 원본 `008.md`의 3:4는 초기 창작 캔버스일 뿐 현재 Skill의 숨은 기본값이 아닙니다.
...
- **독립형 네 작품:** 각 기기는 원본 사진만 참조하며 공간 은유, 경로, 높이 관계, 파스텔 조합과 점프색 위치를 탐색합니다.
...
자동 문구는 보이는 감정, 관계, 동작, 공간 긴장 또는 근거 있는 이야기에서 여운이 남는 짧은 한 문장을 만듭니다. 기본은 대상 언어 하나이며 이중 언어는 명시적으로 요청할 때만 사용합니다.

문구는 대상 언어의 한 문장뿐입니다. 연도, 날짜, 번호, 권호를 더하지 않고 두 번째 언어는 사용자가 명시적으로 요청할 때만 넣습니다.
...
이미지 모델이 원본 전용 공간 은유, 정사영 아이소메트릭, 동적 파스텔, 하나의 점프색, 극도로 깨끗한 무광 3D와 작은 기하 주석을 만듭니다. `scripts/compose_panel.py`는 캔버스 계획, 정확한 50/50 래스터 합성, 최종 크기와 검수만 수행합니다.
...
- [Skill 워크플로](SKILL.md)
- [중문 전체 프롬프트](references/xxd-panel-008-prompt.zh-CN.md)
- [영문 전체 프롬프트](references/xxd-panel-008-prompt.en.md)
