...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-공간시-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 008 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 008 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 008 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 008 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 008의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 008 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 008 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 008 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 008 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 정사영 아이소메트릭 · 플랫폼／계단／문 · 공간 역설 · 동적 파스텔 · 무광 3D

XXD Panel 008는 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진의 알아볼 수 있는 정체성, 윤곽, 자세와 관계를 플랫폼, 계단, 문, 아치, 다리, 통로, 부유 구조와 불가능한 연결로 번역합니다.
...
**Panel 008**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 밝고 고요한 파스텔 불가능 공간으로 번역합니다**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 008으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 008이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 밝고 고요한 파스텔 불가능 공간으로 번역합니다 |
| **한눈에 알아볼 특징** | 정사영 아이소메트릭 · 플랫폼／계단／문 · 공간 역설 · 동적 파스텔 · 무광 3D |
...
```text
원본 사실 고정 → 정체성과 관계를 공간 기하에 배정 → 높이／경로／연결／가림／고립／반복으로 서사 구성 → 정사영 아이소메트릭 고정 → 2–4개 파스텔과 하나의 점프색 선택 → 극도로 깨끗한 무광 3D로 마감
```

무관한 사진으로 바꿔도 작은 삽화의 윤곽, 핵심 색면, 여백 관계와 문구가 거의 그대로 성립한다면 008가 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **원본 전용 공간 은유:** 세 가지 이상의 정체성, 자세, 동작, 기능, 감정 또는 관계 단서가 하나의 기하계에 들어갑니다.
- **기하에 역할이 있음:** 플랫폼, 계단, 문, 아치, 다리, 통로, 부유 구조가 모두 원본 사실에 대응합니다.
- **공간 서사:** 높이는 거리, 경로는 동작, 다리와 통로는 관계, 가림·고립·반복은 감정과 리듬을 표현합니다.
- **정사영 아이소메트릭:** 사진 원근, 붐비는 도시, 미로 과잉, 기성 게임 레벨을 피합니다.
- **동적 파스텔:** 크림, 사암, 분홍, 보라, 파랑, 청록, 민트 중 2–4색으로 유사색과 한난 대비를 만듭니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 008의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-008 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-008 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-008 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-008 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-008 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-008 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 008 적용, 공통 설정은 한 번만 확정
/xxd-panel-008 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-008`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-008-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-008-prompt.en.md)
...
**현실은 기억을 남기고, 기하 공간은 그것을 걸어갈 수 있는 감정으로 바꿉니다.**
